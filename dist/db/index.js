...
import { CREATE_INQUIRIES_TABLE, CREATE_PROPERTIES_TABLE, MIGRATE_INQUIRIES_READ_AT, MIGRATE_PROPERTIES_CREATED_AT, rowToProperty, } from "./schema.js";
...
export function createProperty(input) {
  ...
  `);
    stmt.run(slug, input.title, input.location, input.address, input.badge ?? "DESTAQUE", input.image, JSON.stringify(input.gallery), input.beds, input.baths, input.parking, input.area, input.price, input.priceValue, JSON.stringify(input.description), JSON.stringify(input.features), createdAt);
